...
## 2025-05-17 - Custom Input Affordance and Focus States
**Learning:** Removing default browser styling with \`appearance-none\` on \`select\` elements requires a custom SVG background to maintain visual affordance. Additionally, visually hidden inputs with custom UI overlays must use \`peer-focus-visible\` to provide clear feedback for keyboard users.
**Action:** Always provide a custom icon for \`appearance-none\` selects and ensure hidden inputs have accessible focus-visible rings on their decorative counterparts.
