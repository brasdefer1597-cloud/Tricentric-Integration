...
**Learning:** Native `alert()` calls are disruptive and break the immersive "Sensory Atmosphere" of SRAP. Replacing them with a globally positioned, auto-dismissing `StatusFeedback` component maintains the industrial/neon aesthetic while providing non-blocking guidance.
**Action:** Use the `StatusFeedback` component for all form validations and system notifications to ensure a consistent, high-contrast feedback loop.
