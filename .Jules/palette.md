...
## 2025-05-18 - Proactive Feedback vs. Silent Disabling
**Learning:** Disabling action buttons when form requirements are unmet (without explanation) causes user frustration and confusion. Keeping buttons enabled and providing immediate, actionable error feedback via a non-blocking toast system improves usability and guides the user toward completion.
**Action:** Use the `StatusFeedback` component to replace native `alert()` calls and provide context when users interact with incomplete forms.
