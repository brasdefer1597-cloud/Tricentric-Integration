...
## 2025-05-17 - Non-Blocking Integrated Feedback
**Learning:** Native `alert()` calls disrupt the immersive atmosphere of themed interfaces and are not accessible. Integrated non-blocking notifications using `role="status"` and Tailwind animations provide a seamless user experience.
**Action:** Replace all native `alert()` calls with a component-level `feedback` state, an auto-dismiss `useEffect`, and a `role="status"` container for notifications.
