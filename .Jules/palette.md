...
## 2025-05-17 - Restoring Visual Affordance for Custom Controls
**Learning:** Using `appearance-none` on standard HTML elements like `<select>` removes native visual cues (like the dropdown arrow), which can confuse users and reduce the element's affordance.
**Action:** When overriding native browser styles for form controls, always restore visual affordance using custom SVG icons and ensure high-contrast focus states (e.g., `focus-visible:ring`) for keyboard accessibility.
