...
## 2025-05-17 - Non-blocking Feedback & Form Accessibility
**Learning:** Replacing native `alert()` with an integrated `role="status"` feedback notification improves UX by keeping the user within the application's visual context and avoiding disruptive modal dialogs. Additionally, ensuring custom checkboxes have `peer-focus-visible` indicators is critical for keyboard accessibility when using hidden native inputs.
**Action:** Use a component-level `feedback` state with auto-dismiss logic and Tailwind's `peer-focus-visible` for custom form controls to ensure a smooth and accessible experience.
