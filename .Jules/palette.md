...
## 2025-05-17 - High-Contrast Focus Indicators and Custom Affordance
**Learning:** In dark, neon-industrial themes, default browser focus rings are often invisible. Explicitly using `focus-visible:ring-yellow-400` with `ring-offset-black` ensures accessibility without clashing with the brand. Additionally, when using `appearance-none` on selects, a custom SVG chevron is required to maintain the "dropdown" affordance for users.
**Action:** Use `focus-visible:ring-yellow-400` for keyboard indicators and always provide custom icons for native elements with suppressed styling.
