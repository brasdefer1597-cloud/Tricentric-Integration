...
export default function TricentricIntegration({ kofiUrl }: Props) {
  ...
  const [statusFeedback, setStatusFeedback] = useState<{ message: string; type: 'success' | 'error' | 'info' } | null>(null);
  ...
  const finalizePractice = async () => {
    ...
    const missingFields = Object.entries(responses).filter(([_, val]) => !val.trim());
    if (missingFields.length > 0) {
        setStatusFeedback({
            message: `Please complete all centers and the synthesis before finalizing.`,
            type: 'info'
        });
        return;
    }
    ...
    try {
      ...
      setStatusFeedback({
        message: (
            <>
                Practice finalized and progress saved! Redirecting to Ko-fi...
                <br />
                <a href={kofiUrl} target="_blank" rel="noopener noreferrer" className="underline font-black mt-2 inline-block">
    ...
    <div className="tricentric-integration p-6 max-w-6xl mx-auto bg-gray-900 rounded-2xl border border-purple-900 my-12">
        ...
        {CENTERS.map((center) => (
          ...
          >
            ...
            <textarea
              ...
              onChange={(e) => setResponses({ ...responses, [center.id]: e.target.value })}
      ...
      <div className="bg-gradient-to-r from-blue-900 via-red-900 to-green-900 rounded-2xl p-8 text-center border border-yellow-600">
        ...
        <textarea
          ...
          onChange={(e) => setResponses({ ...responses, synthesis: e.target.value })}
