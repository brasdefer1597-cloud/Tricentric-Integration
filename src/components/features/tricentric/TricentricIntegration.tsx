...
export default function TricentricIntegration({ kofiUrl }: Props) {
  ...
  const [statusFeedback, setStatusFeedback] = useState<{ message: string | React.ReactNode; type: FeedbackType } | null>(null);
    ...
    try {
      ...
      setStatusFeedback({
        message: (
          <span>
            Practice finalized!{' '}
            <a href={kofiUrl} target="_blank" rel="noopener noreferrer" className="underline hover:no-underline">
              Click here
            </a>{' '}
            if the Ko-fi window didn't open.
          </span>
        ),
        type: 'success',
      });
  ...
  return (
    <div className="tricentric-integration p-6 max-w-6xl mx-auto bg-gray-900 rounded-2xl border border-purple-900 my-12">
      {statusFeedback && (
        <StatusFeedback message={statusFeedback.message} type={statusFeedback.type} onClose={closeFeedback} />
      )}
      ...
      <div className="bg-gray-800 rounded-2xl p-8 mb-8 text-center border border-gray-700">
        ...
        <div className="mb-6 text-gray-300 font-bold uppercase tracking-widest text-sm" role="status" aria-live="polite">
