...
export default function TricentricIntegration({ kofiUrl }: Props) {
  ...
  const finalizePractice = async () => {
    if (!userId) {
      setStatusFeedback({ message: 'Please log in to save your progress.', type: 'info' });
      return;
    }
    ...
    try {
      ...
      setStatusFeedback({
        message: (
          <span>
            Practice finalized! <a href={kofiUrl} target="_blank" rel="noopener noreferrer" className="underline font-bold">Click here</a> if not redirected.
          </span>
        ),
        type: 'success'
      });
