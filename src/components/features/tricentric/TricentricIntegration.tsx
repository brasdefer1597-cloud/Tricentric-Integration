...
export default function TricentricIntegration({ kofiUrl }: Props) {
  ...
  const [feedback, setFeedback] = useState<{ message: string | React.ReactNode; type: FeedbackType } | null>(null);
  ...
  const finalizePractice = async () => {
    ...
    try {
      ...
      setFeedback({
        message: (
          <span>
            Practice finalized and progress saved! Redirecting to Ko-fi. If it didn't open,{' '}
            <a href={kofiUrl} target="_blank" rel="noopener noreferrer" className="underline hover:text-white transition-colors">
              click here
            </a>.
          </span>
        ),
