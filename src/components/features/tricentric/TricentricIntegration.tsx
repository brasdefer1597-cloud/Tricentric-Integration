...
export default function TricentricIntegration({ kofiUrl }: Props) {
  ...
  return (
    <div className="tricentric-integration relative p-6 max-w-6xl mx-auto bg-gray-900 rounded-2xl border border-purple-900 my-12">
      {feedback && (
        <div
          role="status"
          className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-xl font-bold text-white shadow-2xl transition-all duration-300 animate-in fade-in slide-in-from-top-4 ${
      ...
      <div className="bg-gray-800 rounded-2xl p-8 mb-8 text-center border border-gray-700">
        ...
        <div
          role="img"
          aria-label={`Breathing exercise: ${breathingPhase}`}
          className={`breathing-circle w-32 h-32 bg-gradient-to-br from-blue-400 to-green-400 rounded-full mx-auto mb-6 flex items-center justify-center transition-all duration-[2000ms] ease-in-out ${
            !intervalId
