...
export default function TricentricIntegration({ kofiUrl }: Props) {
  ...
  const finalizePractice = async () => {
    if (!userId) {
      setStatusFeedback({
        message: 'You are invisible to the system. Log in to save your progress.',
        type: 'error'
      });
      return;
    }
    ...
    try {
      ...
      setStatusFeedback({
        message: (
          <span>
            ...
            <a href={kofiUrl} target="_blank" rel="noopener noreferrer" className="ml-2 underline font-bold">
      ...
      console.error(err);
      setStatusFeedback({
        message: 'The abyss swallowed your progress. Error saving.',
        type: 'error'
      });
    } finally {
      setLoading(false);
