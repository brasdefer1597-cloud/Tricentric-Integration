...
const ExamSection: React.FC<ExamSectionProps> = ({ onEvaluationComplete }) => {
  ...
  const handleAnalyzeSynthesis = async () => {
    if (!synthesis.trim()) {
      alert('You must write a synthesis before analyzing it.');
      return;
    }
  ...
  const handleSaveEvaluation = async () => {
    if (!bleeding || !sacrifice || !synthesis.trim()) {
      alert('You must diagnose the wound, choose a sacrifice, and write a synthesis to seal reality.');
      return;
    }
