...
const ExamSection: React.FC<ExamSectionProps> = ({ onEvaluationComplete }) => {
  ...
  const handleAnalyzeSynthesis = async () => {
    if (!synthesis.trim()) {
      setStatusFeedback({
        message: 'Synthesis is empty. Your truth needs words.',
        type: 'error',
      });
      return;
    }
  ...
  const handleSaveEvaluation = async () => {
    if (!bleeding || !sacrifice || !synthesis.trim()) {
      setStatusFeedback({
        message: 'Incomplete reality. Diagnose, sacrifice, and synthesize first.',
        type: 'error',
      });
      return;
    }
  ...
  const closeStatusFeedback = useCallback(() => setStatusFeedback(null), []);
