...
const ExamSection: React.FC<ExamSectionProps> = ({ onEvaluationComplete }) => {
  ...
  const handleAcceptReality = async () => {
    if (!bleeding || !sacrifice) {
      setStatusFeedback({ message: 'Diagnosis and sacrifice required.', type: 'error' });
      return;
    }
  ...
  const handleSaveEvaluation = async () => {
    if (!bleeding || !sacrifice || !synthesis.trim()) {
      setStatusFeedback({ message: 'Incomplete reality. All fields required.', type: 'error' });
      return;
    }
    ...
    if (result.success) {
      setBleeding(''); setSacrifice(''); setOxygen([]); setSynthesis(''); setAiAnalysis(null);
  ...
  return (
    <>
      {statusFeedback && <StatusFeedback {...statusFeedback} onDismiss={() => setStatusFeedback(null)} />}
