...
const ExamSection: React.FC<ExamSectionProps> = ({ onEvaluationComplete }) => {
  ...
  const handleAnalyzeSynthesis = async () => {
    if (!synthesis.trim()) {
      setFeedback({
        message: 'Enter your synthesis before analysis.',
        type: 'error'
      });
      return;
    }

    const analysisFeedback = await analyze({
      type: 'synthesis',
      synthesis
    });

    setAiAnalysis(analysisFeedback);
    setIsModalOpen(true);
  };
  ...
  const handleSaveEvaluation = async () => {
    if (!bleeding || !sacrifice || !synthesis.trim()) {
      setFeedback({
        message: 'Diagnosis, sacrifice, and synthesis are required to seal reality.',
        type: 'error'
      });
      return;
    }
                ...
                {OXYGEN_OPTIONS.map(opt => (
                  ...
                  >
                    <div className="relative flex items-center">
                      ...
                      />
                      <div className="w-6 h-6 border-2 border-gray-600 rounded-md peer-checked:bg-red-500 peer-checked:border-red-500 transition-all flex items-center justify-center text-black font-bold peer-focus-visible:ring-2 peer-focus-visible:ring-yellow-400 peer-focus-visible:ring-offset-2 peer-focus-visible:ring-offset-black">
                        {oxygen.includes(opt) && '✓'}
                      </div>
                    </div>
