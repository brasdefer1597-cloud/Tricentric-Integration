...
const ExamSection: React.FC<ExamSectionProps> = ({ onEvaluationComplete }) => {
  ...
  const [statusFeedback, setStatusFeedback] = useState<{ message: React.ReactNode; type: 'success' | 'error' | 'info' } | null>(null);
  ...
  const handleAnalyzeSynthesis = async () => {
    if (!synthesis.trim()) {
      setStatusFeedback({
        message: 'Input your survival statement first.',
        type: 'error'
      });
      return;
    }
  ...
  const handleSaveEvaluation = async () => {
    if (!bleeding || !sacrifice || !synthesis.trim()) {
      setStatusFeedback({
        message: 'Incomplete reality. Ensure diagnosis, sacrifice, and synthesis are present.',
        type: 'error'
      });
      return;
    }
