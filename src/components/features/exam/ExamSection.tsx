...
const ExamSection: React.FC<ExamSectionProps> = ({ onEvaluationComplete }) => {
  ...
  const handleAnalyzeSynthesis = async () => {
    if (!synthesis.trim()) {
      setFeedback({
        message: 'Synthesis cannot be empty.',
        type: 'error'
      });
      return;
    }

    const aiFeedback = await analyze({
      type: 'synthesis',
      synthesis
    });

    setAiAnalysis(aiFeedback);
    setIsModalOpen(true);
  };
  ...
  const handleSaveEvaluation = async () => {
    if (!bleeding || !sacrifice || !synthesis.trim()) {
      setFeedback({
        message: 'You must complete the diagnosis, sacrifice, and synthesis.',
        type: 'error'
      });
      return;
    }
