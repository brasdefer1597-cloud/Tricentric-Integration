...
const ExamSection: React.FC<ExamSectionProps> = ({ onEvaluationComplete }) => {
  ...
  const handleAnalyzeSynthesis = async () => {
    if (!synthesis.trim()) {
      setFeedback({ message: 'The abyss requires words. Write your synthesis.', type: 'error' });
      return;
    }

    const feedbackRes = await analyze({
      type: 'synthesis',
      synthesis
    });

    setAiAnalysis(feedbackRes);
    setIsModalOpen(true);
  };

  const handleSaveEvaluation = async () => {
    if (!bleeding || !sacrifice || !synthesis.trim()) {
      setFeedback({ message: 'Your sacrifice is incomplete. Diagnose, choose sacrifice, and write synthesis.', type: 'error' });
      return;
    }
    ...
    <>
      ...
      <section className="py-12 px-6 max-w-4xl mx-auto" id="exam-form">
        ...
        {feedback && (
          <div
            role="status"
            className={`mb-6 p-4 rounded-xl border font-bold text-sm animate-in fade-in slide-in-from-top-4 duration-300 ${
