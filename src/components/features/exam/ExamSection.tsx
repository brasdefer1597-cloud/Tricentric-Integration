...
const ExamSection: React.FC<ExamSectionProps> = ({ onEvaluationComplete }) => {
  ...
  const handleAnalyzeSynthesis = async () => {
    if (!synthesis.trim()) {
      setStatusFeedback({
        message: 'Synthesis is empty. The abyss needs words.',
        type: 'error',
      });
      return;
    }
            ...
            <div className="mt-12 p-8 bg-gradient-to-br from-red-950/20 to-black border border-red-900/50 rounded-3xl relative overflow-hidden group">
                ...
                {statusFeedback && (
                  <div
                    role="status"
                    aria-live="polite"
                    className={`mt-6 p-4 rounded-xl font-black text-center uppercase tracking-widest text-sm animate-in fade-in slide-in-from-top-4 duration-300 ${
