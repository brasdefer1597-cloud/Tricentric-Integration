...
const ExamSection: React.FC<ExamSectionProps> = ({ onEvaluationComplete }) => {
  ...
  const handleAnalyzeSynthesis = async () => {
    if (!synthesis.trim()) {
      setFeedback({ message: 'The synthesis cannot be empty. Speak your truth.', type: 'error' });
      return;
    }
  ...
  const handleSaveEvaluation = async () => {
    if (!bleeding || !sacrifice || !synthesis.trim()) {
      setFeedback({ message: 'Diagnosis, sacrifice, and synthesis are required to seal reality.', type: 'error' });
      return;
    }
  ...
  return (
    <>
      ...
      <section className="py-12 px-6 max-w-4xl mx-auto relative" id="exam-form">
        {feedback && (
          <div
            role="status"
            className={`fixed top-24 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-xl font-bold shadow-2xl animate-in fade-in slide-in-from-top-4 duration-300 ${
                ...
                {OXYGEN_OPTIONS.map(opt => (
                  ...
                  >
                    <div className="relative flex items-center">
                      ...
                      />
                      <div className="w-6 h-6 border-2 border-gray-600 rounded-md peer-checked:bg-red-500 peer-checked:border-red-500 peer-focus-visible:ring-2 peer-focus-visible:ring-yellow-500 peer-focus-visible:ring-offset-2 peer-focus-visible:ring-offset-black transition-all flex items-center justify-center text-black font-bold">
                        {oxygen.includes(opt) && '✓'}
                      </div>
                    </div>
