...
const ExamSection: React.FC<ExamSectionProps> = ({ onEvaluationComplete }) => {
  ...
  const handleSaveEvaluation = async () => {
    if (!bleeding || !sacrifice || !synthesis.trim()) {
      setStatusFeedback({
        message: 'You must complete all steps before sealing reality.',
        type: 'error'
      });
      return;
    }
      ...
      <section className="py-12 px-6 max-w-4xl mx-auto" id="exam-form">
        {statusFeedback && (
          <div
            role="status"
            aria-live="polite"
            className={`mb-8 p-6 rounded-2xl border-2 shadow-xl animate-in fade-in slide-in-from-top-4 duration-500 ${
              statusFeedback.type === 'success'
                ? 'bg-green-950/30 border-green-500 text-green-400 shadow-green-900/20'
                : 'bg-red-950/30 border-red-500 text-red-400 shadow-red-900/20'
            }`}
          >
                ...
                {OXYGEN_OPTIONS.map(opt => (
                  ...
                  >
                    <div className="relative flex items-center">
                      ...
                      />
                      <div className="w-6 h-6 border-2 border-gray-600 rounded-md peer-checked:bg-red-500 peer-checked:border-red-500 peer-focus-visible:ring-2 peer-focus-visible:ring-yellow-500 peer-focus-visible:ring-offset-2 peer-focus-visible:ring-offset-gray-900 transition-all flex items-center justify-center text-black font-bold">
                        {oxygen.includes(opt) && '✓'}
                      </div>
                    </div>
