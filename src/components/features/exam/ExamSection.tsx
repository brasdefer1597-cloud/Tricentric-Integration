...
const ExamSection: React.FC<ExamSectionProps> = ({ onEvaluationComplete }) => {
  ...
  const [statusFeedback, setStatusFeedback] = useState<{ message: React.ReactNode; type: FeedbackType } | null>(null);
  ...
  const handleCloseFeedback = useCallback(() => setStatusFeedback(null), []);
  ...
  const handleAnalyzeSynthesis = async () => {
    if (!synthesis.trim()) {
      setStatusFeedback({
        message: 'The abyss remains silent if you do not provide your synthesis.',
        type: 'info'
      });
      return;
    }
  ...
  const handleSaveEvaluation = async () => {
    if (!bleeding || !sacrifice || !synthesis.trim()) {
      setStatusFeedback({
        message: 'Incomplete reality. Check diagnosis, sacrifice and synthesis.',
        type: 'error'
      });
      return;
    }
