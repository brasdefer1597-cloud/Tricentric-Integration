...
const ExamSection: React.FC<ExamSectionProps> = ({ onEvaluationComplete }) => {
  ...
  const handleAnalyzeSynthesis = async () => {
    if (!synthesis.trim()) {
      setFeedback({ message: 'The synthesis cannot be empty.', type: 'error' });
      return;
    }
  ...
  const handleSaveEvaluation = async () => {
    if (!bleeding || !sacrifice || !synthesis.trim()) {
      setFeedback({ message: 'Incomplete diagnosis. Ensure you selected a wound, a sacrifice, and wrote a synthesis.', type: 'error' });
      return;
    }
          ...
          <div className="space-y-8">
            ...
            <div className="bg-gray-800/50 rounded-2xl p-6 border border-gray-700/50 backdrop-blur-sm">
              ...
              <div className="space-y-3" role="group" aria-label="Oxygen actions">
                {OXYGEN_OPTIONS.map(opt => (
                  <label
                    key={opt}
                    className={`flex items-center space-x-4 p-4 rounded-xl cursor-pointer transition-all border focus-within:ring-2 focus-within:ring-yellow-400 focus-within:ring-offset-2 focus-within:ring-offset-black ${oxygen.includes(opt) ? 'bg-red-950/30 border-red-500 shadow-inner' : 'bg-gray-900/50 border-gray-700 hover:border-gray-600'}`}
                  >
            ...
            <div className="mt-12 p-8 bg-gradient-to-br from-red-950/20 to-black border border-red-900/50 rounded-3xl relative overflow-hidden group">
                ...
                {feedback && (
                  <div
                    role="status"
                    className={`mt-6 p-4 rounded-xl text-center font-bold text-sm animate-in fade-in slide-in-from-top-4 duration-300 ${
