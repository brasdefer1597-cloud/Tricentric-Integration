...
const ExamSection: React.FC<ExamSectionProps> = ({ onEvaluationComplete }) => {
  ...
  const [statusFeedback, setStatusFeedback] = useState<{ type: FeedbackType; message: string } | null>(null);
  ...
  const handleAcceptReality = async () => {
    if (!bleeding || !sacrifice) {
      setStatusFeedback({
        type: 'error',
        message: 'You must diagnose the wound and choose a sacrifice before decoding.'
      });
      return;
    }
  ...
  const handleAnalyzeSynthesis = async () => {
    if (!synthesis.trim()) {
      setStatusFeedback({
        type: 'error',
        message: 'The truth cannot be empty. Write your synthesis first.'
      });
      return;
    }
  ...
  const handleSaveEvaluation = async () => {
    if (!bleeding || !sacrifice || !synthesis.trim()) {
      setStatusFeedback({
        type: 'error',
        message: 'Incomplete reality. Ensure all steps are completed before sealing.'
      });
      return;
    }
