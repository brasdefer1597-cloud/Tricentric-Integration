...
const ExamSection: React.FC<ExamSectionProps> = ({ onEvaluationComplete }) => {
  ...
  const handleAnalyzeSynthesis = async () => {
    if (!synthesis.trim()) {
      setStatusFeedback({
        message: 'You must provide a synthesis statement.',
        type: 'error',
      });
      return;
    }
  ...
  const handleSaveEvaluation = async () => {
    if (!bleeding || !sacrifice || !synthesis.trim()) {
      setStatusFeedback({
        message: 'All fields (Diagnosis, Sacrifice, and Synthesis) are required to seal reality.',
        type: 'error',
      });
      return;
    }
      ...
      <section className="py-12 px-6 max-w-4xl mx-auto" id="exam-form">
        ...
        <div className="bg-gradient-to-br from-gray-900 to-black rounded-2xl p-8 border border-red-900 shadow-2xl shadow-red-900/10">
          ...
          {statusFeedback && (
            <div
              role="status"
              className={`mb-8 p-4 rounded-xl text-sm font-bold text-center animate-in fade-in slide-in-from-top-4 duration-300 ${
                statusFeedback.type === 'success' ? 'bg-green-600 text-white' :
                statusFeedback.type === 'error' ? 'bg-red-600 text-white' :
